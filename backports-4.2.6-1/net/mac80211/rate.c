...
int ieee80211_rate_control_register(const struct rate_control_ops *ops)//record algotithm to reg
{
...
void ieee80211_rate_control_unregister(const struct rate_control_ops *ops)//delete algorithm from reg
{
    struct rate_control_alg *alg;
...
ieee80211_try_rate_control_ops_get(const char *name)//get algo ops according to name from reg
{
    struct rate_control_alg *alg;
...
static void rc_send_low_basicrate(s8 *idx, u32 basic_rates,
                  struct ieee80211_supported_band *sband)//here we can adjust the idx maybe used to indicate the low basic rate
{
    u8 i;
    //printk("yezeliang123, basic_rates = %d", basic_rates);
    if (basic_rates == 0)
